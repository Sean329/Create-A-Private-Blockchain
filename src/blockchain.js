...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            //If it's empty chain then no need to validateChain 
            //just add the genesis block directly
            if (this.height === -1) {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const lapsedTime = currentTime - requestTime;
        ...
        return new Promise(async (resolve, reject) => {
            for (let block of self.chain) {
                if (await block.validate()) {
                    if (block.height > 0) {
                        let prevBlock = self.chain.filter(p => p.height === block.height-1)[0];
